...
async def scrape_tournament_data(tournament_url, age_group, draw_size, sort, tournament_level, pdf_dir=None):
    playwright, browser, context, page = await setup_browser()
    ...
    # allow caller to override output directory (default: ./output)
    if pdf_dir is None:
        pdf_dir = os.path.join(os.getcwd(), "output")
    ...
    # Return generated PDF path (we don't auto-open on the server).
    return pdf_path
